...
// ─── Constants ────────────────────────────────────────────────────────────────
const BACKEND_URL = "https://moral-governing-extension-minister.trycloudflare.com/api/fetch-stream";
const SUBS_URL    = "https://moral-governing-extension-minister.trycloudflare.com/api/subs";
...
// SRT → WebVTT converter (browser-side)
...
const Badge = ({ children, className = "" }) => (
  <span
    className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded text-[9px] font-black uppercase tracking-widest ${className}`}
  >
    {children}
  </span>
);
...
const CastCard = ({ member }) => (
  <div className="flex-shrink-0 w-24 group cursor-default">
    ...
    <p className="mt-2 text-[10px] font-bold text-center truncate text-white/80 group-hover:text-white transition-colors">
      {member.name}
    </p>
    <p className="text-[9px] text-center truncate text-white/40">
      {member.character}
    </p>
  </div>
);
...
const RelatedCard = ({ item, mediaType, onClick }) => {
  ...
  return (
    <div
      ...
      className="group cursor-pointer relative rounded-lg overflow-hidden aspect-[2/3] bg-zinc-900 border border-white/5 hover:border-red-500/50 transition-all duration-300 hover:scale-[1.03] hover:shadow-xl hover:shadow-red-600/10"
    >
      {item.poster_path ? (
        <img
      ...
      <div className="absolute bottom-0 left-0 right-0 p-3 translate-y-2 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-300">
        <p className="text-[10px] font-black uppercase tracking-wide truncate text-white">
          {title}
        </p>
      ...
      <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
        <div className="p-1.5 bg-red-600 rounded-full shadow-lg">
...
const StatTile = ({ icon: Icon, label, value }) => (
  <div className="flex flex-col gap-1 p-4 rounded-lg bg-white/[0.03] border border-white/[0.06] hover:bg-white/[0.06] transition-colors">
    <p className="text-[9px] uppercase tracking-widest opacity-40 flex items-center gap-1.5">
      <Icon size={10} />
      {label}
    </p>
...
const ScoreRing = ({ score }) => {
  ...
  const color =
    pct >= 70 ? "#ef4444" : pct >= 50 ? "#f59e0b" : "#6b7280";
  if (!num) return null;
  return (
...
const MovieDetails = () => {
  ...
  const fetchResumePoint = async (uid) => {
    try {
      const key =
        resolvedMediaType === "tv" ? `tv_${id}_%` : `movie_${id}`;
    ...
    try {
      ...
      for (let attempt = 0; attempt <= MAX_AUTO_RETRIES; attempt++) {
        ...
        try {
          ...
          if (!res.ok) {
            if (res.status === 401) { setError("Authentication failed. Please refresh the page."); return; }
            if (res.status === 429) { setError("Too many requests. Please wait a moment and try again."); return; }
            continue; // other HTTP errors — keep retrying
          }
          ...
          // success=false → server not ready yet, loop around
        } catch (fetchErr) {
          if (fetchErr.name === "AbortError") return;
          // network error → keep retrying
        }
      }
      setError("Stream could not be found. Please try again.");
    } catch (outerErr) {
      if (outerErr.name === "AbortError") return;
      setError("Could not reach the server. Please try again.");
    } finally {
      if (!abortController.signal.aborted) setIsCleaning(false);
    }
  };
  ...
  const tabs = [
    ...
    ...(related.length
      ? [{ key: "related", label: "Related", icon: Film }]
      : []),
  ];

  return (
    <div
      className="relative min-h-screen bg-[#070707] text-white selection:bg-red-600 selection:text-white overflow-x-hidden"
    ...
    >
      ...
      >
        <ArrowLeft
          size={14}
          className="group-hover:-translate-x-1 transition-transform text-white/70 group-hover:text-white"
        />
        <span className="text-[9px] font-black uppercase tracking-widest text-white/60 group-hover:text-white hidden sm:inline">
          Back
        </span>
      </button>
      ...
      <div className="relative h-screen min-h-[600px] max-h-[900px] overflow-hidden">
        <div className="absolute inset-0 w-full h-full scanlines">
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(to right, rgba(7,7,7,0.97) 30%, rgba(7,7,7,0.4) 70%, transparent)",
            }}
          />
          <div
            className="absolute inset-0"
        ...
        <div className="relative z-10 h-full flex items-center px-6 md:px-16 lg:px-24">
          <div className="flex gap-10 items-end w-full max-w-6xl">
            <div
              className="hidden md:block flex-shrink-0 fade-up"
              style={{ width: 180 }}
            >
              <div
            ...
            <div className="flex-1 space-y-5 pb-8">
              <div className="flex flex-wrap gap-2 fade-up">
                ...
                {genre.split(" · ").slice(0, 2).map((g) => (
                  <Badge
                    key={g}
                    className="bg-white/5 border border-white/10 text-white/40"
                  >
                    {g}
                  </Badge>
                ))}
              ...
              <p className="text-sm leading-relaxed text-white/60 max-w-lg line-clamp-3 fade-up-d3">
                {plot}
              </p>
              ...
              <div className="flex flex-wrap items-center gap-3 fade-up-d4">
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-5 py-3.5 rounded-lg text-xs font-black uppercase tracking-widest transition-all hover:scale-[1.02] active:scale-[0.98] ${
      ...
      <div className="px-6 md:px-16 lg:px-24 py-12 space-y-10">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 fade-up-d5">
          <StatTile
            icon={Calendar}
            label="Release"
            value={badgeYear || first_air_date || release_date}
          />
                    ...
                    return (
                      <div
                        ...
                        className={`group cursor-pointer rounded-xl overflow-hidden transition-all duration-300 ${
                          isActive
                            ? "ring-2 ring-red-600 shadow-lg shadow-red-600/20 scale-[1.02]"
                            : "hover:scale-[1.02]"
                        }`}
                        style={{
                          background: "var(--glass)",
                          border: `1px solid ${
                            isActive ? "transparent" : "var(--glass-border)"
                          }`,
                        }}
                      >
                        <div className="relative aspect-video overflow-hidden">
                          ...
                          <div
                            className="absolute top-2 left-2 px-2 py-0.5 rounded text-[9px] font-black text-white/80"
                            style={{
                          ...
                          {isActive && (
                            <div
                              className="absolute top-2 right-2 flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-black text-white"
                        ...
                        <div className="p-3 space-y-1">
                          <h3 className="text-[11px] font-bold uppercase tracking-wide truncate text-white/90 group-hover:text-white transition-colors">
                            {ep.name}
                          </h3>
                          {ep.runtime && (
            ...
            {activeTab === "cast" && cast.length > 0 && (
              <div className="relative">
                <button
                  ...
                  className="absolute left-0 top-8 -translate-x-3 z-10 p-2 rounded-full bg-black border border-white/10 hover:border-white/30 hover:bg-white/10 transition-all"
                >
                  <ChevronLeft size={16} />
                </button>
                <div
                  ref={castScrollRef}
                  className="flex gap-6 overflow-x-auto no-scrollbar pb-4 px-2"
                >
                  {cast.map((m) => (
                ...
                <button
                  onClick={() => scrollCast(1)}
                  className="absolute right-0 top-8 translate-x-3 z-10 p-2 rounded-full bg-black border border-white/10 hover:border-white/30 hover:bg-white/10 transition-all"
                >
                    ...
                    {relatedTotal > 1 && (
                      <div className="flex items-center justify-center gap-2 pt-4">
                        ...
                        <div className="flex items-center gap-1">
                          {Array.from(
                            { length: Math.min(relatedTotal, 7) },
                            (_, i) => {
                              const pg =
                                relatedTotal <= 7
                                  ? i + 1
                                  : relatedPage <= 4
                                  ? i + 1
                                  : relatedPage >= relatedTotal - 3
                                  ? relatedTotal - 6 + i
                                  : relatedPage - 3 + i;
                              return (
                                <button
                                  key={pg}
                                  onClick={() => fetchRelated(pg)}
      ...
      {(activeStream || isPlaying) && (
        ...
        >
          <div
            className="absolute top-0 w-full px-4 md:px-8 py-4 flex items-center justify-between z-[110] pointer-events-none"
            style={{
              background:
          ...
          <div className="flex-grow w-full relative">
            {isCleaning ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-black gap-4">
                ...
                <p className="text-[10px] uppercase tracking-[0.3em] text-white/30 font-bold animate-pulse">
                  Fetching Stream…
                </p>
              </div>
            ) : error ? (
                ...
                >
                  ...
                  {activeStream && (
                    <div className="relative">
                      <button
                        onClick={() =>
                          setActiveMenu((p) =>
                            p === "quality" ? null : "quality"
                          )
                        }
                        className={`px-3 py-2 rounded-lg text-[10px] uppercase font-black text-white transition-all ${
                          activeMenu === "quality"
                            ? "bg-red-600"
                            : "hover:bg-white/10"
                        }`}
                      ...
                      {activeMenu === "quality" && (
                        <div
                          className="absolute bottom-full right-0 mb-2 rounded-lg overflow-hidden min-w-[140px] shadow-2xl thin-scroll"
                          style={{
                        ...
                        >
                          <p className="px-4 py-2 text-[8px] uppercase tracking-widest text-white/20 border-b border-white/5">
                            Quality
                          </p>
                          {[
                            { label: "Auto", idx: -1 },
                            ...
                            <button
                              ...
                              className={`w-full px-4 py-2.5 text-[10px] uppercase text-left hover:bg-red-600/80 transition-colors ${
                                selectedQuality === idx
                                  ? "text-red-400 font-black bg-red-600/20"
                                  : "text-white/60"
                              }`}
                            >
                              {label}
                  ...
                  {subtitleTracks.length > 0 && (
                    <div className="relative">
                      ...
                      {activeMenu === "subs" && (
                        <div
                          className="absolute bottom-full right-0 mb-2 rounded-lg overflow-hidden min-w-[160px] max-h-56 shadow-2xl thin-scroll overflow-y-auto"
                          style={{
                            background: "rgba(10,10,10,0.97)",
                            border: "1px solid rgba(255,255,255,0.1)",
                            backdropFilter: "blur(20px)",
                        ...
                        >
                          ...
                          {subtitleTracks.map((t, i) => (
                            ...
                            >
                              {getLanguageName(t.language, languages)}
                              {t.title ? ` · ${t.title}` : ""}
                            </button>
                          ))}
                        </div>
